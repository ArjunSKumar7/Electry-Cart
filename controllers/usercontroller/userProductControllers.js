...
const {
  viewAddCategory,
} = require("../../helpers/adminHelpers/adminProductHelpers");
...
module.exports = {
  shopProduct: async (req, res) => {
    ...
    if (req.query.catName === undefined) {
      ...
      userproductHelpers
        .shopListProduct(catFilter, page, perpage)
        .then((response) => {
          res.render("user/shop", {
            ...
            cartcount,
            viewCategory,
            pages: Math.ceil(prodlistcount / perpage),
          });
        });
    } else {
  ...
  search:async(req,res)=>{
    ...
    await userproductHelpers.searchByKey(req.body.keyword).then((response)=>{
      res.json(response)
    })
...
sort:async(req,res)=>{
  ...
  await userproductHelpers.sortByPrice(req.query.sortvalue).then((response)=>{
    res.json(response)
  })
